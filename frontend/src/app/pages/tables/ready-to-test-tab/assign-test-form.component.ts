...
export class AssignTestPopoverFormComponent implements OnInit {
  incidentsIds: number[] = [];
  notification = {
    message: "",
    is_read: false,
  ...
  ngOnInit(): void {
    this.incidentService.getIncidentList().subscribe((incidents) => {
      const openIncidents = incidents.filter((incident) => 
        incident.status === "Ready_to_Test" && incident.resolvedBy === null
      );
    
      this.incidentsIds = [...this.incidentsIds, ...openIncidents.map((incident) => incident.id)];
    });
    
    this.userService.getUsersByRoleId(4).subscribe((data) => {
      this.resolvers = data;
    });
  }
  ...
  showSuccess() {
    this.toastrService.show("Test Task assigned successfully ", "Success", {
      status: "success",
    });
  }
              ...
              .subscribe(
                () => {
                    ...
                    this.userService.getUserById(this.assignedTo.id).subscribe((res) => {
                        this.notification.message =  "A new test task has been assigned to you" ;
